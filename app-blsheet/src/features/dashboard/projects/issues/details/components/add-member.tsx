...
const AddMember = ({ issueId, projectId, refetchIssues }: AddMemberProps) => {
  const [open, setOpen] = useState(false);
  const { members } = useGetMembers({});
  const { mutate } = useAssignMember({ refetchIssues });
          ...
          members?.map((member: Member) => (
            <DropdownMenuItem
              ...
              onSelect={() => {
                ...
                mutate({ data: { memberId: member._id, projectId, issueId } });
                setOpen(false);
              }}
            >
              {member.email}
